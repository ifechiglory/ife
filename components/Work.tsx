      ...
      <div className="pointer-events-none absolute inset-0 z-4 flex flex-col justify-center bg-[rgba(10,13,11,0.78)] p-6 opacity-0 backdrop-blur-[2px] transition-opacity duration-350ms group-hover:pointer-events-auto group-hover:opacity-100">
        ...
        <div className="mb-4 flex flex-wrap gap-1.5">
          {project.tools.map((tool) => (
            <span
              key={tool}
              className="rounded-full border border-on-dark-line bg-black/20 px-2.5 py-3 font-mono text-[10px] text-on-dark backdrop-blur-sm"
            >
              {tool}
            </span>
          ))}
        ...
        <div className="flex gap-2.5">
          {hasLiveUrl(project.liveUrl) && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 rounded-sm bg-paper px-3.5 py-2 font-mono text-[11px] text-bg transition-colors"
            >
              Visit site ↗
            </a>
          )}
          {project.githubUrl && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 rounded-sm border border-on-dark-line px-3.5 py-2 font-mono text-[11px] text-on-dark transition-colors hover:text-pine"
            >
